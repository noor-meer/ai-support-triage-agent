...
⚙️ How It Works
Trigger: Gmail inbox is monitored for new incoming support emails
AI Classification: Google Gemini (gemini-2.5-flash) reads the email and returns a structured classification category, urgency, sentiment, confidence score, and reasoning
Routing Decision: An IF node checks whether the ticket is simple and safe enough to auto-handle (category = general question AND confidence ≥ 70%)
Escalation Path: If not safe to auto-handle, a detailed alert (with the AI's full reasoning) is sent to a dedicated Slack channel for a human agent to review
Auto-Handle Path:
...
Logging: Every ticket regardless of path is logged to a Google Sheet with full classification data, for auditability and performance tracking over time
...
AI models can fabricate specific facts even when told not to in general terms — being explicit and specific in guardrail instructions (naming exact categories to avoid guessing) measurably improved output honesty, and RAG eliminates this risk entirely for anything covered in the knowledge base
Embedding model output size must exactly match the vector database's configured dimensions. A default embedding model (text-embedding-004) turned out to be deprecated; the replacement (gemini-embedding-001) outputs 3072-dimension vectors, requiring the Pinecone index to be rebuilt to match — always verify actual output size rather than trusting a default
Vector search results return as multiple separate items, not one bundled item. This caused a "paired item data unavailable" error several nodes downstream, because every node after the search was unexpectedly running once per retrieved chunk instead of once per ticket. The fix: merge all retrieved chunks into a single item ($input.all().map(...).join(...)) immediately after the vector search step, before any node that expects one item per original ticket
...
A workflow being "Active" does not mean every configuration is correct — always verify with a fully hands-off, real-world test (no manual node execution) before considering a build production-ready
📋 Setup Instructions
Import the workflow JSON into your n8n instance
Create a Pinecone account and index (dimension must match your chosen embedding model's output size — verify this rather than assuming)
Connect your own Gmail, Google Gemini, Pinecone, Slack, and Google Sheets credentials
Upload your own knowledge base content using the "Add documents to vector store" flow (a one-time setup step, separate from the live workflow)
Update the Slack channel name and Google Sheet reference to your own
